...
export const isValidNumberLength = (
    ...
    isNumber(value, error_message);

    if (isNullOrUndefined(String(value))) {
        throw new Error(error_message);
    }

    if (value < min) throw new Error(error_message);
    if (max !== undefined && value > max) throw new Error(error_message);
...
export const isValidBithDate = (
    value: Date | string,
    error_message: string
) => {
    const date = dayjs(value);

    if (date.isValid() === false && dayjs().diff(date, "year") < 18)
        throw new Error(error_message);
