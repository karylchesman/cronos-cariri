...
export type TSearchObjectOperator =
    | '='
    | '<>'
    | '<'
    | '>'
...
export function getWhereObject(
    app_operator: TSearchObjectOperator,
    key: string,
    value: string | number | boolean,
    default_alias?: string
) {
    const keyHasJoin = key.split(':');

    const genderWhereString = (operator: string) => {
        if (keyHasJoin.length > 1)
            return `${keyHasJoin[0]}.${keyHasJoin[1]} ${operator} :${keyHasJoin[0]}_${keyHasJoin[1]}`;
    ...
    switch (app_operator) {
        case '=':
            return {
                where_string: genderWhereString('='),
                value_param: { [genderWhereParamName()]: value },
            };
        case '<>':
            return {
                where_string: genderWhereString('!='),
        ...
        case 'A*':
            return {
                where_string: genderWhereString('LIKE'),
                value_param: { [genderWhereParamName()]: `${value}%` },
            };
        case '*A':
            return {
...
export function isValidSearchOperator(operator: any) {
    ...
    if (
        !['=', '<>', '<', '>', '<=', '>=', 'A*', '*A', '*A*'].includes(operator)
    )
        throw new Error('Operador de busca inválido.');
    return operator;
}
