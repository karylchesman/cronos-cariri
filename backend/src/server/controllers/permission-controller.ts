...
class PermissionController {
    ...
    async search(request: Request, response: Response) {
        ...
        const permissionRepository = new PermissionRepository();
        const searchPermissionUsecase = new SearchPermissionUsecase(
            permissionRepository
        );
    ...
    async updatePermission(request: Request, response: Response) {
        const { id, name, identifier } = request.body;
    ...
    async attachPermissionToRole(request: Request, response: Response) {
        const { role_id, permissions_ids } = request.body;

        const roleRepository = new RoleRepository();
        const permissionRepository = new PermissionRepository();
        const rolePermissionRoleRepository = new RolePermissionRepository();

        const attachRoleToUserUseCase = new AttachPermissionToRoleUsecase(
            roleRepository,
            permissionRepository,
            rolePermissionRoleRepository
        );
    ...
    async getByRoleId(request: Request, response: Response) {
        ...
        const getPermissionsByRoleUsecase = new GetPermissionsByRoleUsecase(
            roleRepository,
            rolePermissionRoleRepository,
            permissionRepository
        );
